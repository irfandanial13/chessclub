...
<body class="elite-chess-theme">
    ...
    <div class="auth-container" style="box-shadow:0 4px 24px rgba(139,92,42,0.08);">
        ...
        <form method="post" action="<?= base_url('admin/events/update/'.$event['event_id']) ?>">
            ...
            <input type="date" name="date" class="elite-input" value="<?= esc($event['date']) ?>" style="color:#000; font-size:1.08em; margin-bottom:18px;" required><br>
            <label style="color:#8B5C2A; font-weight:600; font-size:1.08em; margin-bottom:6px;">Location:</label><br>
            <input type="text" name="location" class="elite-input" value="<?= esc($event['location']) ?>" style="color:#000; font-size:1.08em; margin-bottom:18px;" required><br>
